...
json_action_result add_timer_action(std::optional<unsigned int> index, const char *input, size_t input_len, char *output_buffer, size_t output_buffer_len) {
    ...
    std::memcpy(to_add.timer_name.data(), name_token.ptr, std::min(static_cast<int>(name_length), name_token.len));
    ...
    if (to_add.result.collection_result == timer_collection_operation::ok && to_add.result.result_index.has_value()) {
        if (output_buffer != nullptr && output_buffer_len != 0) {
            result.answer_len = json_printf(&answer, "{ index : %d, info : %Q}", *to_add.result.result_index, "Ok added timer");
        }
        result.result = json_action_result_value::successfull;
    } else {
        if (output_buffer != nullptr && output_buffer_len != 0) {
