...
class ScheduleTrackerTwoEntriesTests : public ::testing::Test {
    protected:
    ...
    ScheduleTrackerTwoEntriesTests() : tracker(&schedule, ScheduleEventTransitionMode::SingleShot) {
...
TEST_F(ScheduleTrackerTwoEntriesTests, SingleShotBehavior) {
...
TEST_F(ScheduleTrackerTwoEntriesTests, HoldBehaviorFirstPoint) {
    ...
    // Expect the first event (10:30) since 10:35 > 10:00
    ASSERT_TRUE(value.has_value());
    EXPECT_FLOAT_EQ(value.value(), 1.0f);
...
TEST_F(ScheduleTrackerTwoEntriesTests, HoldBehaviorLastPoint) {
    tracker.setTrackingType(ScheduleEventTransitionMode::Hold);

    // monday.insertTimePoint(11h + 30min, ChannelData{8.0f, 9.0f});

    // Set currentDate to Monday, 10:35:00
    std::tm currentDate = makeTime(1, 12, 35);
    ...
    // Expect the first event (10:30) since 10:35 > 10:00
    ASSERT_TRUE(value.has_value());
    EXPECT_FLOAT_EQ(value.value(), 8.0f);

    auto secondValue = tracker.getCurrentChannelValue(1, currentDate);
    ASSERT_TRUE(secondValue.has_value());
    EXPECT_FLOAT_EQ(secondValue.value(), 9.0f);
...
TEST_F(ScheduleTrackerTwoEntriesTests, InterpolationBehavior) {
...
TEST_F(ScheduleTrackerTwoEntriesTests, InvalidChannelIndex) {
...
/* Repeating schedule will always return event
// Test for No Event Found
TEST_F(ScheduleTrackerTwoEntriesTests, NoEventFound) {
    tracker.setTrackingType(ScheduleEventTransitionMode::SingleShot);
...
TEST_F(ScheduleTrackerTwoEntriesTests, EventAfterWrapAround) {
...
TEST_F(ScheduleTrackerTwoEntriesTests, EventNextDay) {
    struct Event { int eventData; std::chrono::seconds eventTime; };
