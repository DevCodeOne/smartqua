...
class ScheduleTrackerUnevenEventsTests : public ::testing::Test {
    protected:
    ...
    ScheduleTrackerUnevenEventsTests() : tracker(&schedule, ScheduleEventTransitionMode::SingleShot) {
        using namespace std::chrono_literals;
        TestDayScheduleType monday;
        monday.insertTimePoint(9h + 30min, ChannelData{1.0f});
        monday.insertTimePoint(10h, ChannelData{std::nullopt, 2.0f});
        monday.insertTimePoint(10h + 30min, ChannelData{4.0f, std::nullopt});
...
TEST_F(ScheduleTrackerUnevenEventsTests, SingleShotBehavior) {
...
TEST_F(ScheduleTrackerUnevenEventsTests, HoldBehavior) {

}

// Test for Interpolation Behavior
TEST_F(ScheduleTrackerUnevenEventsTests, InterpolationBehavior) {

}

TEST_F(ScheduleTrackerUnevenEventsTests, EventAfterWrapAround) {
...
TEST_F(ScheduleTrackerUnevenEventsTests, EventNextDay) {
    // struct Event { int eventData; std::chrono::seconds eventTime; };
