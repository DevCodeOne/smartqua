...
public:
    ...
    explicit RestStorage(Args &&... args) : pathGenerator(std::forward<Args>(args) ...) {
    ...
    RestStorage(RestStorage &&other)
            : client(other.client), isValid(other.isValid), restTarget(other.restTarget),
              pathGenerator(other.pathGenerator) {
    ...
    bool retrieveData(char *dst, size_t len) {
        ...
        auto error = esp_http_client_set_header(client, "Content-Type",
                                                DataTypeToContentType<StorageType>::ContentType);
